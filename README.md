...
1 个编排智能体 + 7 个专业智能体，覆盖"信号识别 → 信息连接 → 安全合规"完整链路：
...
- 🧍 **数字人体档案** — 将既有就诊与追加资料按解剖部位归档，在男女 3D 人体模型上查看时间线（只整理、不诊断）
...
- **动态用户管理**：内置画像一键切换，也可新增用户并立即联动聊天、健康功能和数字人体档案
- **连续对话**：通用问答与 7 类专业意图自动路由，会话及消息记录持久化保存
- **3D 解剖档案联动**：复用用户与就诊数据库，支持器官标记、档案聚焦、时间轴回放和追加写入 API
...
│   │   ├── routers/             # 全部 Router（含 eeg、imaging、body_archive）
│   │   ├── services/
...
├── frontend/
│   └── src/
│       ├── app/                 # 页面（含脑电、影像、数字人体档案、权益/报销/政策/安全）
