...
async def db():
    ...
    async with session_factory() as session:
        ...
        session.add(User(id=2, name="李大爷", age=70, gender="男", city="杭州",
...
class TestAppendRecord:
    async def test_append_record_roundtrip(self, client, db):
        resp = await client.post("/api/body-archive/patients/user_001/records", json={
            "organ": "lungs", "event_date": "2026-2", "description": "肺部小结节",
    ...
    async def test_full_date_normalized(self, client):
        resp = await client.post("/api/body-archive/patients/1/records", json={
            "organ": "shoulder_right", "event_date": "2026-07-5",
            "raw_excerpt": "右肩疼",
        })
        assert resp.status_code == 200
...
class TestRegisterMaterial:
    async def test_register_material_shows_in_profile(self, client):
        resp = await client.post("/api/body-archive/patients/user_001/materials", json={
            "filename": "胸部CT.pdf", "note": "复查资料",
        })
        assert resp.status_code == 200
    ...
    async def test_unknown_patient_404(self, client):
        resp = await client.post("/api/body-archive/patients/user_999/materials", json={
            "filename": "x.pdf",
        })
        assert resp.status_code == 404
