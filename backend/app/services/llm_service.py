...
class LLMService:
    ...
    封装 OpenAI 兼容 API，提供医保智脑所需的各项 LLM 能力。
    """
...
只返回 JSON，不要其他内容。"""
    ...
    # RAG 系统提示词模板
    RAG_SYSTEM_TEMPLATE = """你是"医保智脑"——一个专业的医保政策智能助手。你的职责是基于检索到的政策资料，准确、专业地回答用户关于医保政策的问题。
...
- 最后给出建议或注意事项"""
    ...
    # 健康预警系统提示词
    HEALTH_ALERT_PROMPT = """你是医保智脑的健康管理模块。根据用户的健康数据，生成个性化的健康预警和建议。
