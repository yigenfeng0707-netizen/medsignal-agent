...
SERVICE_PATH = REPO_ROOT / "backend" / "app" / "services" / "body_archive.py"
SPEC = importlib.util.spec_from_file_location("medsignal_body_archive", SERVICE_PATH)
if not SPEC or not SPEC.loader:
    raise RuntimeError(f"无法加载数字人体服务模块：{SERVICE_PATH}")
SERVICE = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(SERVICE)
ORGAN_LABELS = SERVICE.ORGAN_LABELS
infer_organ = SERVICE.infer_organ
validate_event_date = SERVICE.validate_event_date
...
def extract_date(text: str) -> str:
    for pattern in DATE_PATTERNS:
        ...
        if not match:
            continue
        year, month, day = match.group(1), match.group(2), match.group(3) if match.lastindex == 3 else None
