...
- API 路由：`backend/app/routers/body_archive.py`
- 分类与序列化：`backend/app/services/body_archive.py`
- 数据模型：`backend/app/models.py` 中的 `BodyArchiveRecord` / `BodyArchiveMaterial`
- 3D 查看器：`backend/app/static/digital-body/index.html`
- GLB 模型：`backend/app/static/digital-body/models/`
- 前端入口：`frontend/src/app/body-archive/page.tsx`
...
```bash
cd backend
python -m pytest tests/test_body_archive.py -q
python -m ruff check app/routers/body_archive.py app/services/body_archive.py tests/test_body_archive.py
